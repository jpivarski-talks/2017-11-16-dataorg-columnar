...
\vspace{0.2 cm}
Currently, we make 2 or 3 levels of ``slimmed'' copies (AOD/MiniAOD/NanoAOD) to serve with different latencies. Three sizes does not fit all, so individual analyses make their own subsets (and have to find their own storage).
...
\vspace{0.2 cm}
\begin{itemize}
\item<1-> Server-bound file view would have many users, ``home directories.''
\item<2-> Adopt object store's security model.
\item<3-> Same baskets would be identified by object store keys. No fragmentation concerns and objects are replicated.
